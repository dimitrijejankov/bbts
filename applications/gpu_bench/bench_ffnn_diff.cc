...
auto ffnn_iter(std::list<bbts::command_ptr_t> &ffnn_commands,
  ...
  auto difference = generate_multiply(scheduler, udf_manager, ffnn_commands, 
      "ffnn_diff_mm_kernel", a_2, a_2, false, true,
      num_batch / batch_block, 
      num_batch / batch_block,
      num_labels / labels_block, 
      bbts::ffnn_add::elementwise_fn_type::NOOP);

  return std::tuple{a_1, a_2, difference};
}
...
int main() {
  ...
  x = generate(scheduler, factory, storage, num_batch,
                      num_features, num_batch / batch_block,
                      num_features / features_block, false);

  y = generate(scheduler, factory, storage, num_batch,
  ...
  auto [a_1, a_2, difference] = ffnn_iter(ffnn_commands, scheduler, udf_manager, x, y, w1, w2);
