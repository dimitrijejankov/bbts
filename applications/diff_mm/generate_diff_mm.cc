...
int main() {
  ...
  int32_t num_nodes;
  int32_t n, m, k;
  int32_t n_split, m_split, k_split;

  // ([n, m] x [m, k])
  std::cout << "the number of nodes : \n";
  std::cin >> num_nodes;
  std::cout << "[n, m] x_diff [m, k])\n";
  std::cout << "n :";
  std::cin >> n;
  std::cout << "n_split :";
  std::cin >> n_split;
  ...
  funs.push_back(abstract_ud_spec_t{.id = DIF_MM_ID,
...

  {
    // [n, m] x [m, k]
    std::vector<abstract_command_t> commands;
    multiply(n_split, k_split, m_split, commands, a_index, b_index, out_index);
