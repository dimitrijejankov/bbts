#include "local_stack_op.h"

bbts::local_stack_op_t::local_stack_op_t(bbts::tensor_factory_t &_factory,
                                           bbts::storage_t &_storage,
                                           const std::vector<tid_t> &_inputs,
                                           const ud_impl_t::tensor_params_t &_params,
                                           bbts::tid_t _out_tid,
                                           const bbts::ud_impl_t &_stack_op) : _factory(_factory),
                                                                                _storage(_storage),
                                                                                _inputs(_inputs),
                                                                                _params(_params),
                                                                                _out_tid(_out_tid),
  ...
  _id = _factory.get_tensor_ftm(_stack_op.outputTypes.front());
}

void bbts::local_stack_op_t::apply() {
  ...
  for(size_t idx = 1; idx < _inputs.size(); ++idx) {
    ...
    _storage.local_transaction({lhs, rhs}, {}, [&](const storage_t::reservation_result_t &res) {
      ...
      // get the meta data
      _stack_op.get_out_meta(_params, _input_meta, _output_meta);

      // set the output meta
      auto &type = _stack_op.outputTypes[0];
    ...
    _storage.local_transaction({lhs, rhs}, {{TID_NONE, output_size}}, [&](const storage_t::reservation_result_t &res) {
      ...
      // run the function
      _stack_op.call_ud(_params, _input_tensors, _output_tensor);
