...
public:
  command_handler_apply_t(class reservation_station_t *_rs);
  ~command_handler_apply_t() override;
  ...
  // the local apply and move commands and the number of tensors they are
  // waiting for
  std::unordered_map<command_id_t, std::pair<command_ptr_t, int32_t>>
      _local_commands;
