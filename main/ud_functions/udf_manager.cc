...
udf_manager_t::udf_manager_t(tensor_factory_ptr_t _tensor_factory, 
                             gpu_scheduler_ptr_t _gpu_scheduler) : _gpu_scheduler(std::move(_gpu_scheduler)),
...
ud_impl_t* udf_manager_t::get_fn_impl(ud_impl_id_t _id) {
  ...
  if(_id.ud_id >= static_cast<int32_t>(_udfs.size()) ||
      _id.impl_id >= static_cast<int32_t>(_udfs[_id.ud_id]->impls.size())) {
    return nullptr;
  }
